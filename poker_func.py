...
__all__ = ["new_game", "join_game", "start_game", "deal_hand", "call_bet", "strip_bet",
           "bind_bet", "check", "raise_bet", "fold_hand", "show_options", "set_option",
           "chip_count", "all_in"]


def new_game(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    ...
    if game.state == GameState.NO_GAME:
        ...
        return [f"A new game has been started by {message.author.display_name}!",
                f"Message {ctx.prefix}join to join the game."]
    else:
        messages = ["There is already a game in progress, "
                    "you can't start a new game."]
        if game.state == GameState.WAITING:
            messages.append("It still hasn't started yet, so you can still "
                            f"message {ctx.prefix}join to join that game.")
        return messages


def join_game(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    ...
    if game.state == GameState.NO_GAME:
        return ["No game has been started yet for you to join.",
                f"Message {ctx.prefix}newgame to start a new game."]
        ...
        return [f"{message.author.display_name} has joined the game!",
                f"Message {ctx.prefix}join to join the game, "
                f"or {ctx.prefix}start to start the game."]
...
def start_game(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    if game.state == GameState.NO_GAME:
        return [f"Message {ctx.prefix}newgame if you would like to start a new game."]
        ...
        return [f"You are not a part of that game yet, {message.author.display_name}.",
                f"Please message {ctx.prefix}join if you are interested in playing."]
...
# should say
def deal_hand(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    if game.state == GameState.NO_GAME:
        return ["No game has been started for you to deal. "
                f"Message {ctx.prefix}newgame to start one."]
    elif game.state == GameState.WAITING:
        ...
        return [f"You aren't the dealer, {message.author.display_name}.",
                f"Please wait for {game.dealer.user.display_name} to {ctx.prefix}deal."]
    else:
...
def call_bet(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    if game.state == GameState.NO_GAME:
        return [f"No game has been started yet. Message {ctx.prefix}newgame to start one."]
    elif game.state == GameState.WAITING:
...
# Handles a player stripping, giving an appropriate error message if the
# user is not the current player or betting hadn't started. Returns the list of
# messages the bot should say.
def strip_bet(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    if game.state == GameState.NO_GAME:
        return [f"No game has been started yet. Message {ctx.prefix}newgame to start one."]
    elif game.state == GameState.WAITING:
        ...
        return [f"You can't strip {message.author.display_name}, because it's "
                f"{game.current_player.user.display_name}'s turn."]
    else:
        return game.strip()


# Handles a player binding, giving an appropriate error message if the
# user is not the current player or betting hadn't started. Returns the list of
# messages the bot should say.
def bind_bet(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    if game.state == GameState.NO_GAME:
        return [f"No game has been started yet. Message {ctx.prefix}newgame to start one."]
    elif game.state == GameState.WAITING:
        return ["You can't bind because the game hasn't started yet."]
    elif not game.is_player(message.author):
        return ["You can't bind, because you're not playing, "
                f"{message.author.display_name}."]
    elif game.state == GameState.NO_HANDS:
        return ["You can't bind because the hands haven't been "
                "dealt yet."]
    elif game.current_player.user != message.author:
        return [f"You can't bind {message.author.display_name}, because it's "
...
def check(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    if game.state == GameState.NO_GAME:
        return [f"No game has been started yet. Message {ctx.prefix}newgame to start one."]
    elif game.state == GameState.WAITING:
        ...
        return [f"You can't check, {message.author.display_name} because you need to "
                f"put in ${game.cur_bet - game.current_player.cur_bet} to call."]
    else:
        return game.check()
...
def raise_bet(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    if game.state == GameState.NO_GAME:
        return [f"No game has been started yet. Message {ctx.prefix}newgame to start one."]
    elif game.state == GameState.WAITING:
    ...
    if len(tokens) < 2:
        return [f"Please follow {ctx.prefix}raise with the amount that you would "
                "like to raise it by."]
    try:
        amount = int(tokens[1])
        ...
        return [f"Please follow {ctx.prefix}raise with an integer. "
...
def fold_hand(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    if game.state == GameState.NO_GAME:
...
# Returns the list of messages the bot should say.
def set_option(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    tokens = message.content.split()
    if len(tokens) == 2:
        return ["You must specify a new value after the name of an option "
                f"when using the {ctx.prefix}set command."]
    elif len(tokens) == 1:
        return ["You must specify an option and value to set when using "
                f"the {ctx.prefix}set command."]
    elif tokens[1] not in GAME_OPTIONS:
        return [f"'{tokens[1]}' is not an option. Message {ctx.prefix}options to see "
                "the list of options."]
    try:
        val = int(tokens[2])
        if val < 0:
...
def chip_count(game: Game) -> List[str]:
...
# to say.
def all_in(game: Game, message: discord.Message, ctx: commands.Context) -> List[str]:
    if game.state == GameState.NO_GAME:
        return [f"No game has been started yet. Message {ctx.prefix}newgame to start one."]
    elif game.state == GameState.WAITING:
